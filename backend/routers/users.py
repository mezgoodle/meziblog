...
from database import UserRead, UserUpdate, get_session, User
...
async def read_users(
    *,
    ...
    try:
        users = get_objects(session, User, offset, limit)
...
async def read_user(
    *,
    ...
    try:
        user = get_object(session, User, user_email, True)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    return user
...
async def update_user(
    *,
    ...
    try:
        updated_user = patch_object(session, user, user_data)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    return updated_user
...
async def delete_user(
    *,
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    try:
        result = delete_object(session, user)
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    return result
